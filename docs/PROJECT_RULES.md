...
> Chi tiết đầy đủ data contracts (verified against DB) → xem `docs/SYSTEM_REFERENCE.md` Section 6.
>
> Nguyên tắc chung:
> - Bronze → Silver: Nếu thiếu column → dbt fail rõ ràng (không silent skip)
> - Silver → Gold: Chỉ lấy `WHERE is_valid = TRUE`
> - Idempotency: `ON CONFLICT (code, date) DO UPDATE` — chạy 2 lần không tăng dòng
