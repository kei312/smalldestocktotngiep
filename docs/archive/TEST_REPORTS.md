...
## Ngày 8 — Daily Check & Báo cáo kỹ thuật

### [Daily] — Pytest toàn bộ (2026-06-24)
- Lệnh:      `cd /home/naeouad/deproject && PYTHONPATH=. ./venv/bin/pytest tests/ -v --tb=short`
- Kết quả:   `10 passed, 4 warnings in 35.53s` (Python 3.12.3, pytest-9.1.1)
- Thời gian: 2026-06-24 14:03
- Trạng thái: ✅ PASS

### [Daily] — dbt test toàn bộ (2026-06-24)
- Lệnh:      `docker exec airflow-container bash -c "cd /opt/airflow/project/dbt && dbt test --profiles-dir ."`
- Kết quả:   `Done. PASS=12 WARN=0 ERROR=0 SKIP=0 NO-OP=0 TOTAL=12` (dbt=1.10.22, 1.39s)
- Thời gian: 2026-06-24 14:03
- Trạng thái: ✅ PASS

### [Vnstock Ratio Optimization] — Thay đổi tỷ lệ xoay vòng nguồn (4 KBS : 1 VCI)
- Lệnh:      `PYTHONPATH=. venv/bin/pytest tests/`
- Kết quả:   `10 passed, 4 warnings in 42.22s`
- Thời gian: 2026-06-24 21:27
- Trạng thái: ✅ PASS

## Ngày 9 — Tối ưu hóa hiệu năng CTE đệ quy (RSI/EMA/MACD)

### [2.3.5 - Optimized] — dbt run gold (Optimized build)
- Lệnh:      `docker exec airflow-container bash -c "cd /opt/airflow/project/dbt && dbt run --select gold --profiles-dir ."`
- Kết quả:   `Done. PASS=9 WARN=0 ERROR=0 SKIP=0 NO-OP=0 TOTAL=9` in 12.87 seconds. Thời gian chạy các model đệ quy giảm từ >15 phút xuống còn ~4.0s (EMA) và ~4.7s (RSI) nhờ pre-calculate và unique B-tree indexes.
- Thời gian: 2026-06-25 04:57 (local time) / 21:57 (UTC)
- Trạng thái: ✅ PASS

### [2.3.7 - Optimized] — dbt test gold (Optimized tests)
- Lệnh:      `docker exec airflow-container bash -c "cd /opt/airflow/project/dbt && dbt test --select gold --profiles-dir ."`
- Kết quả:   `Done. PASS=6 WARN=0 ERROR=0 SKIP=0 NO-OP=0 TOTAL=6` in 0.38 seconds.
- Thời gian: 2026-06-25 04:57 (local time) / 21:57 (UTC)
- Trạng thái: ✅ PASS

### [2.3.8 - Optimized] — G-03 MACD Verification (Full refresh verification)
- Lệnh:      `docker exec airflow-container bash -c "cd /opt/airflow/project/dbt && dbt run --select gold --full-refresh --profiles-dir . && python3 /opt/airflow/project/scripts/verify_macd_g03.py"`
- Kết quả:   `dbt run --full-refresh` chạy thành công toàn bộ 514k dòng trong 16.97s. MACD verification script trả về `ALL PASS` với sai số `0.0000%` cho cả MACD line và MACD signal của cả 3 mã VNM, VCB, HPG.
- Thời gian: 2026-06-25 04:58 (local time) / 21:58 (UTC)
- Trạng thái: ✅ PASS

### [3.3.4 - Upgraded] — dbt run & test fact_stock_indicators (SMA 50/200 & Lookback 360 days)
- Lệnh:      `docker exec airflow-container bash -c "cd /opt/airflow/project/dbt && dbt run --select fact_stock_indicators --full-refresh --profiles-dir . && dbt test --select fact_stock_indicators --profiles-dir ."`
- Kết quả:   `dbt run --full-refresh` rebuild thành công 514,951 dòng với SMA 50/200 và Bollinger Bands 200 ngày. `dbt test` thông qua toàn bộ 3 tests (rsi_14 range, ma200 positive, bb_upper >= bb_lower).
- Thời gian: 2026-06-25 06:02 (local time) / 23:02 (UTC)
- Trạng thái: ✅ PASS

### [3.3.4 - Dashboard] — HTML Dashboard Upgrade with Global Filter Group (All vs VN30)
- Lệnh:      `docker exec airflow-container python3 /opt/airflow/project/scripts/generate_dashboard_backup.py`
- Kết quả:   Sinh thành công file HTML `reports/dashboard_backup.html`. Đã bổ sung bộ chọn nhóm toàn cục "Tất cả" vs "Chỉ VN30" ở header điều phối. Khi chuyển chế độ, toàn bộ Dashboard 1 (KPIs, Pie độ rộng, Top 10), Dashboard 2 (dropdown mã cổ phiếu), Dashboard 3 (độ rộng lịch sử + thanh khoản), và Dashboard 4 (so sánh P/E, ROE, bảng chi tiết + KPIs trung bình) đều tự động cập nhật số liệu và render tương ứng.
- Thời gian: 2026-06-25 06:15 (local time) / 23:15 (UTC)
- Trạng thái: ✅ PASS

### [Seed Restore] — Khôi phục bảng dim_date (dbt seed)
- Lệnh:      `docker exec airflow-container bash -c "cd /opt/airflow/project/dbt && dbt seed --profiles-dir ."`
- Kết quả:   `1 of 1 OK loaded seed file public.dim_date [INSERT 2557 in 1.49s]`. SQL query: `SELECT COUNT(*) FROM public.dim_date;` trả về `2557` dòng.
- Thời gian: 2026-06-25 12:03 (UTC)
- Trạng thái: ✅ PASS

## Ngày 10 — Mô phỏng Cơ chế Ngắt mạch (Circuit Breaker)

### [5.4.2] — Mô phỏng Kịch bản C (Vi phạm Schema Tầng Silver)
- Lệnh:      Chèn "mã độc" (`NULL AS symbol`) vào `silver_prices.sql`, sau đó chạy `dbt run --select silver_prices` và `dbt test --select silver_prices`.
- Kết quả:   Bài test `not_null` và `unique_combination_of_columns` lập tức báo **FAIL 508190**. Airflow Task chuyển sang màu Đỏ, cắt đứt hoàn toàn luồng cấp dữ liệu xuống tầng Gold. (Đã khôi phục lại mã nguồn gốc sau khi chụp log).
- Thời gian: 2026-06-26 12:30 (UTC)
- Trạng thái: ✅ PASS (Hệ thống ngắt mạch bảo vệ hoạt động chính xác theo đúng Kịch bản C).

### [5.4.2] — Mô phỏng Kịch bản D (Sai lệch Logic Tầng Gold)
- Lệnh:      Chèn "mã độc" cộng thêm 200 vào công thức `rsi_14` trong `fact_stock_indicators.sql`, sau đó chạy `dbt run --full-refresh --select fact_stock_indicators` và `dbt test --select fact_stock_indicators`.
- Kết quả:   Bài test generic `expression_is_true_fact_stock_indicators_rsi_14_0_AND_rsi_14_100` lập tức phát hiện dị thường và báo **FAIL 502570**. Tác vụ `dbt_test_gold` Failed (Màu đỏ), ngăn chặn Power BI cập nhật dữ liệu rác. (Đã khôi phục lại công thức gốc).
- Thời gian: 2026-06-26 12:40 (UTC)
- Trạng thái: ✅ PASS (Hệ thống ngắt mạch bảo vệ hoạt động chính xác theo đúng Kịch bản D).

### [Cập nhật] — Bollinger Bands (20 phiên) Verification
- Lệnh:      `dbt run --select fact_stock_indicators --full-refresh` & `dbt test`
- Kết quả:   `Done. PASS=3 WARN=0 ERROR=0 SKIP=0 NO-OP=0 TOTAL=3`. Bài test `bb_upper >= bb_lower` cùng với các test khác hoàn tất thành công sau khi cập nhật công thức `rn >= 20`.
- Thời gian: 2026-06-26 19:54 (UTC)
- Trạng thái: ✅ PASS

## Ngày 11 — Rà soát báo cáo & Đồng bộ Benchmark

### [5.7 - Align] — Đồng bộ Throughput & Benchmark toàn sàn HoSE (403 mã)
- Lệnh:      `PYTHONPATH=. PROVIDER=mock venv/bin/python3 -m ingestion.fetch_prices --mode all --start 2021-01-01 --end 2026-06-26`
- Kết quả:   MockProvider chạy hoàn toàn offline bằng cách đọc file CSV tĩnh nên hoàn tất rất nhanh chỉ trong **3.7 giây** cho 40 mã mock (57.000 dòng dữ liệu).
- Lưu ý: Chỉ số benchmark **8.5 phút** và giới hạn thông lượng **~50 req/phút** trong báo cáo là các thông số thực nghiệm ghi nhận được khi chạy **VnstockProvider thật** qua mạng internet đến KBS & VCI (bị giới hạn bởi RateLimiter 1.5s/request cho mỗi nguồn và ThreadPool 5 workers).
- Thời gian: 2026-06-27 00:53 (UTC)
- Trạng thái: ✅ PASS

## Ngày 12 — Kiểm thử bổ sung các trường hợp biên (NaN, Empty Response, Mock Fallback)

### [5.2 - Edge Cases] — Pytest Suite cho các trường hợp biên dữ liệu
- Lệnh:      `PYTHONPATH=. venv/bin/pytest tests/`
- Kết quả:   `14 passed, 4 warnings in 27.81s`. Các test case mới bao gồm:
  - `test_mock_provider_future_date_fallback`: Kiểm chứng cơ chế tự phục hồi tự động khi request ngày tương lai nằm ngoài fixture (trả về dòng cuối và gán date = end).
  - `test_vnstock_empty_response`: Xác minh VnstockProvider xử lý phản hồi rỗng từ API một cách trơn tru, không crash và trả về DataFrame trống.
  - `test_vnstock_nan_symbol`: Xác minh VnstockProvider ném đúng lỗi `ProviderError` khi gặp symbol không tồn tại hoặc mang giá trị `NaN`.
- Thời gian: 2026-06-27 01:22 (UTC)
- Trạng thái: ✅ PASS

### [5.1 - Verification] — dbt test execution after schema update
- Lệnh:      `docker exec airflow-container bash -c "cd /opt/airflow/project/dbt && dbt test --profiles-dir ."`
- Kết quả:   `Done. PASS=12 WARN=0 ERROR=0 SKIP=0 NO-OP=0 TOTAL=12` hoàn tất trong 1.68 giây. Toàn bộ 12 tests (generic tests bao gồm unique_combination_of_columns, expression_is_true cho rsi_14, ma200, bb_upper, và các test not_null) đều vượt qua thành công trên cơ sở dữ liệu thực tế.
- Thời gian: 2026-06-27 01:39 (UTC)
- Trạng thái: ✅ PASS

## Ngày 12 (Tiếp tục) — Kiểm chứng độc lập với thư viện chuẩn Python 'ta'

### [5.2 - Cross Verification] — verify_macd_talib.py
- Lệnh:      `python3 tests/verify_macd_talib.py`
- Kết quả:   Đối chiếu chéo Gold layer data (VCB, VNM, HPG) với thư viện Python `ta` (Bukosabino) sau khi bỏ qua 250 phiên warm-up.
  - HPG: RSI14 MAPE = 0.000053%, MACD Line/Signal MAPE = 0.000000%
  - VCB: RSI14 MAPE = 0.000053%, MACD Line/Signal MAPE = 0.000000%
  - VNM: RSI14 MAPE = 0.000054%, MACD Line/Signal MAPE = 0.000000%
  - Sai số RSI cực kỳ nhỏ do khác biệt làm tròn số thực double precision của PostgreSQL vs Python. MACD hoàn hảo 100%.
- Thời gian: 2026-06-27 02:20 (UTC)
- Trạng thái: ✅ PASS

## Ngày 12 (Tiếp tục) — Khắc phục lỗi báo cáo & Tích hợp Quality Gate

### [Report - Quality Gate] — dbt run & dbt test Silver Layer
- Lệnh:      `docker exec airflow-container bash -c "cd /opt/airflow/project/dbt && dbt run --select silver --profiles-dir ."` và `dbt test --select silver`
- Kết quả:   `dbt run`: PASS=2 models (silver_prices build thành công 510.841 dòng trong 8.04s, silver_index build 2727 dòng). `dbt test`: PASS=6 tests.
- Xác minh:   Truy vấn SQL ghi nhận có 82 bản ghi bị gán cờ `dq_flag = 'abnormal_price_gap'` do biến động giá đóng cửa >15% trong khi VNINDEX biến động <3%.
- Thời gian: 2026-06-27 02:48 (UTC)
- Trạng thái: ✅ PASS

### [Report - Quality Gate] — dbt run & dbt test Gold Layer
- Lệnh:      `docker exec airflow-container bash -c "cd /opt/airflow/project/dbt && dbt run --select gold --profiles-dir ."` và `dbt test --select gold`
- Kết quả:   `dbt run`: PASS=9 models (fact_stock_price nhận đúng 510.759 dòng sạch, loại bỏ hoàn toàn 82 dòng lỗi). `dbt test`: PASS=6 tests.
- Thời gian: 2026-06-27 02:48 (UTC)
- Trạng thái: ✅ PASS

## Ngày 12 (Tiếp tục) — Rà soát báo cáo & Đánh giá chất lượng sau làm sạch

### [Report - Clean up] — Pytest Suite Verification after report standardization
- Lệnh:      `PYTHONPATH=. venv/bin/pytest tests/`
- Kết quả:   `14 passed, 4 warnings in 27.59s` (Toàn bộ 14 test cases pass, xác minh hệ thống hoạt động ổn định không có regression).
- Thời gian: 2026-06-27 06:11 (UTC)
- Trạng thái: ✅ PASS

### [Report - Clean up Phase 2.5] — Pytest Suite Verification after final adjustments
- Lệnh:      `PYTHONPATH=. venv/bin/pytest tests/`
- Kết quả:   `14 passed, 4 warnings in 30.15s` (Toàn bộ 14 test cases pass).
- Thời gian: 2026-06-27 06:39 (UTC)
- Trạng thái: ✅ PASS

## Ngày 12 (Tiếp tục) — Cải tiến logic đếm Gainer/Loser tài chính & Đồng bộ số liệu báo cáo

### [M-05] — Cập nhật logic SQL fact_market_summary
- Lệnh:      `docker exec airflow-container bash -c "cd /opt/airflow/project/dbt && dbt run --select fact_market_summary --profiles-dir ."`
- Kết quả:   `Done. PASS=1 WARN=0 ERROR=0 SKIP=0 NO-OP=0 TOTAL=1` (1430 rows created in 0.63s).
- Thời gian: 2026-06-27 14:21
- Trạng thái: ✅ PASS

### [Pytest - Verification] — Pytest Suite cho toàn bộ kịch bản
- Lệnh:      `PYTHONPATH=. ./venv/bin/pytest tests/`
- Kết quả:   `15 passed, 5 warnings in 28.72s` (Toàn bộ 15 test cases passed).
- Thời gian: 2026-06-27 14:22
- Trạng thái: ✅ PASS

### [dbt test - Verification] — dbt test toàn bộ mô hình Gold
- Lệnh:      `docker exec airflow-container bash -c "cd /opt/airflow/project/dbt && dbt test --profiles-dir ."`
- Kết quả:   `Done. PASS=12 WARN=0 ERROR=0 SKIP=0 NO-OP=0 TOTAL=12` (12 tests passed, including fact_market_summary check).
- Thời gian: 2026-06-27 14:25
- Trạng thái: ✅ PASS

### [5.7.4 - RTO Verification] — dbt build RTO Measurement
- Lệnh:      `docker exec airflow-container bash -c "cd /opt/airflow/project/dbt && dbt build --full-refresh --profiles-dir ."`
- Kết quả:   `Done. PASS=24 WARN=0 ERROR=0 SKIP=0 NO-OP=0 TOTAL=24`. Hoàn thành `dbt build` tầng Silver và Gold kèm tất cả test chỉ trong 32.88 giây (tối ưu hóa do thêm chỉ mục và cache query hiệu quả).
- Thời gian: 2026-06-27 16:40 (UTC)
- Trạng thái: ✅ PASS

## Ngày 13 — Cập nhật bảng chiều thời gian dim_date phục vụ Power BI Slicer

### [Power BI - Calendar Slicer] — Thêm cột 'day' (ngày trong tháng) vào dim_date seed
- Lệnh:      `python3 scripts/generate_dim_date.py` & `docker exec airflow-container bash -c "cd /opt/airflow/project/dbt && dbt seed --full-refresh --profiles-dir ."`
- Kết quả:   
  - `dim_date.csv` được tạo lại, bổ sung cột `day` (chứa giá trị `dates.day`).
  - `dbt seed --full-refresh` chạy thành công: `1 of 1 OK loaded seed file public.dim_date ..................................... [CREATE 11323 in 5.03s]`.
  - SQL verification: Bảng `public.dim_date` bổ sung cột `day` kiểu `integer` và các giá trị ngày từ 1-31.
- Thời gian: 2026-06-30 19:31 (UTC)
- Trạng thái: ✅ PASS

## Ngày 14 — Update GitHub & Verify Application

### [GitHub Update] — Pull code & Fast-Forward merge
- Lệnh:      `git fetch origin && git merge origin/main`
- Kết quả:   Fast-forwarded thành công 1 commit mới nhất (`01d4957643b0` - fix env). Không xảy ra conflict với local changes.
- Thời gian: 2026-07-07 14:26 (UTC)
- Trạng thái: ✅ PASS

### [Application Verify] — Chạy Pytest & dbt build kiểm tra hệ thống
- Lệnh:      `PYTHONPATH=. venv/bin/pytest tests/` & `docker exec airflow-container bash -c "cd /opt/airflow/project/dbt && dbt build --profiles-dir ."`
- Kết quả:   
  - Pytest: `16 passed, 7 warnings in 31.14s`.
  - dbt build: `Done. PASS=24 WARN=0 ERROR=0 SKIP=0 NO-OP=0 TOTAL=24` hoàn thành trong 25.52 giây (dbt=1.10.19, xử lý ~493k dòng dữ liệu).
  - Airflow DAGs: Cả 2 DAGs `daily_stock_pipeline` và `manual_backfill_pipeline` đều được parse thành công không có lỗi cú pháp.
- Thời gian: 2026-07-07 14:28 (UTC)
- Trạng thái: ✅ PASS
